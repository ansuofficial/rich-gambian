...
import { ChartSkeleton } from "@/app/components/chart-skeleton";
import { SHARE_ENABLED } from "@/utils/constants/constants";
import { ReceiptData } from "@/app/types/types";

const SpendingChart = dynamic(
  () =>
    import("@/app/components/spending-chart").then((m) => m.SpendingChart),
  { ssr: false, loading: () => <ChartSkeleton /> }
);

const ShareReceiptDialog = dynamic(
  () =>
    import("@/app/components/share-receipt-dialog").then(
      (m) => m.ShareReceiptDialog
    ),
  { ssr: false }
);

export function GdpSpender() {
  const {
    balance,
    purchases,
    totalSpent,
